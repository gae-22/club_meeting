...
function dbw_update_search(dbw_id) {
    ...
    for (i = 0; i < cols; i++) {
        ...
        if (form[dbw_id + 'filter' + i]) {
            ...
            if (filter[i] == '[all]') dofilter[i] = false;
            if (filter[i] == '[empty]') filter[i] = '';
        }
    }
    ...
    for (i = 1; i < table.rows.length; i++) {
        ...
        if (show) table.rows[i].style.display = '';
...
function showStartStopTimes(startTime) {
    ...
    var tStart =
        'auto scroll debug on -- Initialization start H:M:S:ms=' +
...
function setSpanTags(isPreview) {
    ...
    if (bottom) {
        ...
        if (autoScrollDebugOn) {
            alert(
                "auto scroll debug 1: document.getElementById('bottom') failed"
            );
        }
    }
    var skipTo = -1;
            ...
            if (lineParts.length == 3) {
                ...
                for (var k = 0; k < nbrParsedLines; ++k) {
                    if (
                        ...
                        if (autoScrollDebugOn) {
                            alert(
                                'auto scroll debug 2: skipping walkDom, i=' +
                                    i +
                                    ' k=' +
                                    k +
                                    ' marks[i].id=' +
                ...
                if (marks[i + 1] && marks[i + 1].id) {
                    ...
                    if (autoScrollDebugOn) {
                        alert(
                            'auto scroll debug 3: skipping walkDom, i=' +
                                i +
                                ' marks[i].id=' +
                                marks[i].id
                        );
